      ...
      {isAuthenticated && !loading ? (
        <>
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md text-center">
            <h1 className="text-2xl font-bold mb-4 text-orange-600">Welcome {userData.name}</h1>
            <p className="text-gray-700">Email: <span className="text-black">{authData.email}</span></p>
            {userData.verified ? (
            ...
            {limit && (
              <p className="text-gray-700">Search Token Remaining: <span className="text-black">{limit.remaining_quota}</span></p>
            )}
            <button 
              onClick={logout} 
        ...
        !loading && !error && !isAuthenticated && <p className="text-xl text-gray-700">Please login</p>
      )}
    </div>
  )
}
